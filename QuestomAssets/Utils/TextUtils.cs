...
namespace QuestomAssets.Utils
{
    ...
    {
        public static Dictionary<string, Dictionary<string, string>> ReadLocaleText(string text)
        {
            // Assume the first line is as follows, ALWAYS: STRING ID,DESCRIPTION,LANGUAGE1,LANGUAGE 2 / 2ALIAS, ... \r\n (or \n)
            var seps = new List<char>();
            var headerList = new List<string>();
            // Read header
            string header = text.Split(new string[] { "\n" }, StringSplitOptions.None)[0]; // Version 1.0.0 - 1.0.2 Support
            if (header.EndsWith("\r")) // Version 1.1.0+ Support
                header = text.Split(new string[] { "\r\n" }, StringSplitOptions.None)[0];
